...
export function DashboardPage() {
  ...
  return <><section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-signal"></p><h1 className="mt-1 text-3xl font-bold tracking-tight text-ink">Welcome back, {user?.username.split(/[._ -]/)[0] ?? 'there'}.</h1></div><Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4" />New link</Button></section>{error ? <div className="mt-6"><InlineError message={error} onRetry={() => void load()} /></div> : <><section className="mt-7"><StatsGrid data={overview} loading={loading} /></section><section className="mt-6 rounded-xl border border-line bg-white p-5 shadow-sm sm:p-6"><div className="mb-5"><h2 className="text-lg font-bold text-ink">Clicks over the last 30 days</h2></div><ClickAnalyticsChart data={series} loading={loading} /></section><LinksSection onChanged={() => void load()} /></>}<CreateLinkDialog open={createOpen} onClose={() => setCreateOpen(false)} onCreated={created} /></>
}
